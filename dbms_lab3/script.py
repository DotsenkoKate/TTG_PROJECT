...
server_uri = "mongodb+srv://cyber_essence:<password>@cluster0.ha8y8.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
...
def insert_fields_user():
    ...
    for i in range(num_of_records):
        db.col.insert_one({"cat_id": input_user()[0], "login": input_user()[1], "password": input_user()[2], "name": input_user()[3]})

def insert_fields_owner():
    num_of_records = int(input('Введите кол-во вставляемых записей: '))
    for i in range(num_of_records):
        db.col.insert_one({"id": input_owner()[0], "source_type": input_owner()[1], "text": input_owner()[2]})

def insert_fields_trip():
    num_of_records = int(input('Введите кол-во вставляемых записей: '))
    for i in range(num_of_records):
        db.col.insert_one({"id": input_trip()[0], "way_number": input_trip()[1], "date": input_trip()[2], "auto_number": input_trip()[3], "start": input_trip()[4], "finish": input_trip()[5]})

def insert_fields_divide():
    num_of_records = int(input('Введите кол-во вставляемых записей: '))
    for i in range(num_of_records):
        db.col.insert_one({"id": input_divide_of_trip()[0], "trip_id": input_divide_of_trip()[1], "station_id": input_divide_of_trip()[2], "time": input_divide_of_trip()[3]})
...
#аналитический запрос - добавить маршрут
#insert_fields_trip()
